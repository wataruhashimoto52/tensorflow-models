...
def inference(images):
    ...
    with tf.name_scope("conv1") as scope:
        # 3*3*1のフィルタで畳み込み，output を64チャネルに
    ...
    with tf.name_scope("conv2") as scope:
        kernel = _variable_with_weight_decay("weights", shape=[5,5,64,64],
                                            stddev=5e-2,wd=None)
        conv = tf.nn.conv2d(images, kernel,[1,1,1,1], padding="SAME")
        biases = _variable_on_cpu("baises", [64], tf.constant_initializer(0))
        pre_activation = tf.nn.bias_add(conv, biases)
    ...
    with tf.name_scope("fc3") as scope:
        reshape = tf.reshape(norm2, [images.get_shape()[0], -1])
    ...
    with tf.name_scope("fc4") as scope:
        weights = _variable_on_cpu("weights", shape=[384, 192],
                                    stddev=0.04, wd=0.004)
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=labes, logits=logits, name="cross_entropy_per_example"
    )
...
def _add_loss_summaries(total_loss):
    ...
    # do the same for the averaged version of the loss
...
def train(total_loss, global_step):
    ...
    # compute gradients.
    # withステートメント以下2行を実行しようとすると,
    # 先にloss_averages_opが実行される
